...
import { QuestResponseType } from '../lib/types'

type Props = {
  response: QuestResponseType[]
  setActiveQuestId: Dispatch<SetStateAction<string>>
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button className="inline-flex w-full justify-center rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-500 hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
        {response?.find((res) => res.quest.id === activeQuestId)?.quest.title}
        <ChevronDownIcon
      ...
      >
        <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="px-1 py-1 ">
            {response?.map((res) => (
              <Menu.Item key={res.quest.id}>
                {({ active }) => (
                  <button
                    className={`${
                      active ? 'bg-purple-500 text-white' : 'text-gray-900'
                    } group flex w-full items-center rounded-md px-2 py-2 text-sm`}
                    onClick={() => setActiveQuestId(res.quest.id)}
                  >
                    {res.quest.title}
                  </button>
                )}
